...
<!-- Chart always visible — shows empty bars on refresh -->
...
const totalPages = computed(() => Math.max(1, Math.ceil((itemsStore.filteredItems ?? []).length
/ 10)))
...
// REFRESH — Chart stays visible, bars reset to 0
...
await statsStore.fetchStats()
chartRef.value?.reset() // Bars animate down to 0
updateTimestamp()
currentPage.value = 1
errorMessage.value = ''
} catch (error) {
...
chartRef.value?.reset() // Bars grow from 0 to new data
